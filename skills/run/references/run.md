...
4. **Otherwise** (an agent role: `researcher`, `planner`, `test-writer`, `executor`, `verifier`, `drift-detector`, `critic`, `manager`), spawn a subagent via `Agent`:
   ...
   - **Build the run-context block.** Choose the construction by your detected context window:
     - **(a) Full block — the default, and always for the `critic`.** Manifest content + every prior stage's artifact file content (concatenated with `--- <filename> ---` separators). Use this whenever you are NOT on a 1M-context model. Also use it for the `critic` stage at every window: the critic's hostile cold read must not be starved, so it always receives the full prior artifacts inline.
     - **(b) Lean block — only on a 1M-context model, and never for the `critic`.** You are on a 1M-context model if `hooks/hook_utils.py:detect_context_window` would return 1M for your model id — i.e. your id carries a `[1m]` suffix or matches one of the 1M-capable prefixes in that helper's `_CONTEXT_WINDOW_1M_PREFIXES`, which is the single source of truth for the set (read it if unsure; do not keep a second copy of the model list here). Inject the manifest content in full, then an **artifact index**: one line per prior artifact, `<filename> — <one-line description> — .agent-runs/<run-id>/<filename>`. Then instruct the subagent to **read on demand** the specific artifacts it needs from `.agent-runs/<run-id>/`, rather than receiving them inline. The run context stays resident — fully readable on request — without pasting every prior artifact into each fresh subagent's prompt.
...
3. **Classify** the action: run `python scripts/classify_action.py <tool> "<arguments>"` — it prints exactly one of `read_only` / `reversible_write` / `external_facing` / `high_risk`. (`read_only`/`reversible_write` should not arrive here — the executor runs those directly — but if one does, log `auto_allow` to judge-log.yaml, delete `pending-action.yaml`, re-spawn the executor with "your proposed action was reversible; run it directly and continue," and loop.)
...
5. **Parse the verdict** from that file. Act on `verdict`:
   - **`allow` + `external_facing`** → write the exact command to `.agent-runs/<run_id>/judge-approved-next.txt` (the one-shot approval sidecar; the hook honors it for exactly that command and clears it once the action has run), execute the action via Bash, append `judged_allow` to judge-log.yaml.
...
**Floor precedence (preserved, never loosened):** the deterministic PreToolUse hook keeps its absolute `deny` floor for destructive and secret-exposure commands (`rm -rf`, force-push, `DROP TABLE`, `export *KEY=`, …). That floor precedes the judge and cannot be reopened by a judge `allow` or by the approval sidecar — the sidecar exempts only the external-facing and release class the hook redirects with `JUDGE_REVIEW_REQUIRED`, never the destructive/secret deny floor. (Privilege-escalation commands like `sudo` are not on this hard floor; `action-classification.yaml` routes them through the judge as `high_risk` instead.)
