...
4. **Otherwise** (an agent role: `researcher`, `planner`, `test-writer`, `executor`, `verifier`, `drift-detector`, `critic`, `manager`), spawn a subagent via `Agent`:
   ...
   - **Build the run-context block.** Choose the construction by your detected context window:
     ...
     - **(b) Lean block — only on a 1M-context model, and never for the `critic`.** You are on a 1M-context model if your model id is Opus 4.6 / 4.7 / 4.8, Sonnet 4.6, or carries a `[1m]` suffix — the same set `hooks/hook_utils.py:detect_context_window` maps to 1M (keep this list in lockstep with that helper). Inject the manifest content in full, then an **artifact index**: one line per prior artifact, `<filename> — <one-line description> — .agent-runs/<run-id>/<filename>`. Then instruct the subagent to **read on demand** the specific artifacts it needs from `.agent-runs/<run-id>/`, rather than receiving them inline. The run context stays resident — fully readable on request — without pasting every prior artifact into each fresh subagent's prompt.
